/* maauthcoduiajaxsave.i  MEDSTAR Medical Aid System
...
*/                      
  ...
  DEFINE VARIABLE cPMBDecisionRuleValue     AS CHARACTER                   NO-UNDO.
  DEFINE VARIABLE cPMBViewDetailsRuleValue  AS CHARACTER                   NO-UNDO.
  ...
  DEFINE VARIABLE lPMBViewDetailsValidRule  AS LOGICAL                     NO-UNDO.
  DEFINE VARIABLE lPMBDecisionValidRule     AS LOGICAL                     NO-UNDO.
    ...
    DO:
      ...
      */
      IF NOT {&ErrorStatus} THEN
        ASSIGN oSearch = NEW cls.maauthsearch(DATASET dsAuthorisation BY-REFERENCE) NO-ERROR.
    ...
    */
    ...
    DO:
      FOR EACH tt_auth_coding EXCLUSIVE-LOCK:
        ...
        ASSIGN tt_auth_coding.record_action = 'MODIFY':U.
    ...
    /*
      And save
    */
    IF NOT {&ErrorStatus} AND CAN-FIND(FIRST tt_auth_coding) THEN
    ...
    DO:
      ...
      */
      FOR EACH tt_auth_error EXCLUSIVE-LOCK:
        ...
        IF cFieldName <> ? THEN 
          ASSIGN tt_auth_error.error_element_name = cFieldName + STRING(iRow) NO-ERROR.
  ...
  DO:
    ASSIGN
       oRequestHelper  = NEW cls.maajaxrequesthelper (INPUT get-value('FldLst':U), INPUT get-value('ValList':U))       
       ...
       dStartDate      = (IF oRequestHelper:getFieldValue("fdStartDate":U + cContainerCode) MATCHES "*YYYY*":U
                          THEN ?
       ...
       lPmbApplicable  = IF cContainerCode BEGINS "ICDCoding":U
                         THEN CAN-DO("Y,YES,TRUE":U,oRequestHelper:getFieldValue("flPMBIndicator":U  + cContainerCode))
                         ELSE FALSE
        ...
        DO:
          ...
          IF NOT {&ErrorStatus} THEN
            ASSIGN
              ...
              oCoding:RecordAction         = 'PARTIALSAVE':U
          ...
          DO:
            ...
            */
            ...
            DO:
              oDiagnosis = NEW cls.madiagnosis() NO-ERROR.

              oDiagnosis:focusDiagnosis(dAsteriskObj) NO-ERROR.

              IF oDiagnosis:DiagnosisInFocus THEN
                ASSIGN cAsterisk = oDiagnosis:Diagnosis NO-ERROR.
            ...
            */
            ASSIGN lSuccess = mipEnv:Health:AuthMaintenance:getAuthRuleValue
                            (INPUT  dInsurerObj,
                             ...
                             OUTPUT lPMBViewDetailsValidRule,
                             OUTPUT cPMBViewDetailsRuleValue)
                           NO-ERROR.

            IF NOT {&ErrorStatus} AND LOOKUP(TRIM(cPMBViewDetailsRuleValue),"Active,Activate":U) > 0 THEN
            ...
            IF NOT {&ErrorStatus} THEN
              ASSIGN lSuccess = mipEnv:Health:AuthMaintenance:getAuthRuleValue
                              (INPUT  dInsurerObj,
                               ...
                               INPUT  "PMBDecision":U,
                               INPUT  oAuthorisation:StartDate,
                               OUTPUT lPMBDecisionValidRule,
                               OUTPUT cPMBDecisionRuleValue)
                             NO-ERROR.

            /*
              Condition code will be populated if it is a PMB.
            */
            IF NOT {&ErrorStatus}                  AND cContainerCode BEGINS "ICDCoding":U 
            AND cPMBDecisionRuleValue = "System":U AND cConditionCode <> "":U THEN
              ASSIGN oCoding:PmbIndicator = TRUE.

            ASSIGN lSuccess = oCoding:saveRecord() NO-ERROR.
            ...
            */                  
            IF NOT {&ErrorStatus} THEN
              ASSIGN
                ...
                lSuccess                        = oResponseHelper:addFieldValue("fcPMBIndicatorState":U + cContainerCode, cPMBDecisionRuleValue, TRUE)
                lSuccess                        = oResponseHelper:addFieldValue("fcBodyRegion":U        + cContainerCode, oCoding:BodyRegion, TRUE)
               NO-ERROR.

            IF NOT {&ErrorStatus}                  AND cContainerCode BEGINS "ICDCoding":U 
            AND cPMBDecisionRuleValue = "System":U AND cConditionCode <> "":U THEN
              ASSIGN lSuccess = oResponseHelper:addFieldValue("flPMBIndicator":U + cContainerCode, STRING(oCoding:PMBIndicator), TRUE) NO-ERROR.

            IF NOT {&ErrorStatus}       AND oCoding:AssDiagObj <> 0.00 
            AND cOwningAltValue <> "":U AND cAsterisk <> "":U THEN
              ASSIGN
                 lSuccess = oResponseHelper:addFieldValue("fcOwningAltValue":U + cContainerCode, cOwningAltValue + "+":U)
    ...
    IF cErrorMessage <> "":U OR oCoding:ErrorObject:ErrorsExist THEN                       
    DO:
    ...
    END. /*IF cErrorMessage <> "":U OR oCoding:ErrorObject:ErrorsExist THEN*/
  
    ASSIGN lSuccess = oResponseHelper:setError(oCoding:ErrorObject) NO-ERROR.
