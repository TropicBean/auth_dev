        ...
        */
        ...
        FIND FIRST hac_auth_type NO-LOCK
             WHERE hac_auth_type.auth_type       = cAuthTypeCode 
               AND hac_auth_type.auth_type_group = cAuthTypeGroup NO-ERROR.
	{ mip/inc/mipthrowerror.i &IgnoreErrors = 'PROGRESS:565' &ResetIgnoredErrors = TRUE }	       
        ...
        DO:
          ...
          THEN DO:
            ...
            VALIDATE ttValidation.
          END. /* IF lUpdatesAllowed = FALSE */
          
        END. /* IF AVAILABLE hac_auth_type */
        ELSE 
        DO:
          IF NOT CAN-FIND(FIRST hac_auth_type NO-LOCK
                          WHERE hac_auth_type.auth_type = cAuthTypeCode) THEN
          DO:
            ASSIGN
              ttValidation.lValid   = FALSE
          ...
          IF NOT CAN-FIND(FIRST hac_auth_type NO-LOCK
                          WHERE hac_auth_type.auth_type = cAuthTypeCode
                            AND hac_auth_type.auth_type_group = cAuthTypeGroup) THEN
          DO:
            IF NOT CAN-FIND(FIRST tt_acronym
                            WHERE tt_acronym.category_key = "ma_acAuthTypeGroup":U)
            THEN
              mipEnv:Health:maUtility:getAcronymDetails(INPUT "ma_acAuthTypeGroup", INPUT "":U, OUTPUT TABLE tt_acronym BY-REFERENCE).
          
            FIND FIRST tt_acronym 
                WHERE  tt_acronym.acronym_key = cAuthTypeGroup NO-ERROR.
         
            { mip/inc/mipthrowerror.i &IgnoreErrors = 'PROGRESS:565' &ResetIgnoredErrors = TRUE }
            IF AVAILABLE tt_acronym THEN
            ASSIGN
              ttValidation.lValid   = FALSE
              ttValidation.cMessage = SUBSTITUTE("The Authorisation type  &1 is not valid for the Authorisation Type Group &2", UPPER(cAuthTypeCode), UPPER(tt_acronym.acronym_label)).
            VALIDATE ttValidation.

          END. /* IF NOT CAN-FIND(FIRST hac_auth_type NO-LOCK... */
        END. /* ELSE-IF AVAILABLE hac_auth_type */
